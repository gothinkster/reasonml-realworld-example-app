// Generated by BUCKLESCRIPT VERSION 1.8.2, PLEASE EDIT WITH CARE
'use strict';

var Body                              = require("./body.js");
var ReactDOMRe                        = require("reason-react/lib/js/src/reactDOMRe.js");
var ReasonReact                       = require("reason-react/lib/js/src/reasonReact.js");
var Director$slashbuild$slashdirector = require("director/build/director");

function renderForRoute(route) {
  return ReactDOMRe.renderToElementWithId(ReasonReact.element(/* None */0, /* None */0, Body.make(route, /* array */[])), "root");
}

var router = new Director$slashbuild$slashdirector.Router({
      "/": (function () {
          return renderForRoute(/* Home */0);
        }),
      "/home": (function () {
          return renderForRoute(/* Home */0);
        }),
      "/register": (function () {
          return renderForRoute(/* Register */1);
        }),
      "/login": (function () {
          return renderForRoute(/* Login */2);
        }),
      "/settings": (function () {
          return renderForRoute(/* Settings */3);
        }),
      "/article/create": (function () {
          return renderForRoute(/* CreateArticle */5);
        }),
      "/article/edit": (function () {
          return renderForRoute(/* EditArticle */6);
        }),
      "/article": (function () {
          return renderForRoute(/* Article */4);
        })
    });

router.init("/");

exports.renderForRoute = renderForRoute;
exports.router         = router;
/* router Not a pure module */
