...
function loginUser(route, $$event, param) {
  var state = param[/* state */4];
  var reduce = param[/* reduce */3];
  $$event.preventDefault();
  var reduceByAuthResult = function (_, jsonPayload) {
  ...
  JsonRequests.authenticateUser(reduceByAuthResult, user(state));
...
function make(router, _) {
                              ...
                              className: "col-md-6 offset-md-3 col-xs-12"
                                  ...
                                  className: "text-xs-center"
                                      ...
                                      onClick: Curry._1(self[/* handle */0], (function (param, param$1) {
                                              return loginUser(router, param, param$1);
                                            }))
                                    }, "Sign in"))))));
    });
  ...
  newrecord[/* reducer */12] = (function (action, state) {
      if (typeof action === "number") {
        console.log("getting set to pending");
        return /* NoUpdate */0;
      } else {
        switch (action.tag | 0) {
          case 0 : 
              var match = action[0];
              var errorList = match[1];
              console.log("Reducer call with error list: " + (String(errorList) + ""));
